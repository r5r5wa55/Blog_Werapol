...
export default function UpdatePost() {
    ...
    const handleSubmit = async(e)=>{
        e.preventDefault();
        try {
            const res = await fetch(`/api/post/updatepost/${formData._id}/${currenUser._id}`,{
                method:'PUT',
                headers:{   
  ...
  return (
    <div className="max-w-5xl mx-auto p-3 min-h-screen">
        <h1 className="text-center font-sans text-4xl py-5 font-bold">Update a Post</h1>
        <form className='flex flex-col' onSubmit={handleSubmit}>
            <div className="flex flex-col sm:flex-row gap-5">
                <TextInput type="text" name="" id="title" value={formData.title || ''} placeholder='title' className='flex-1'  onChange={(e)=>setFromData({...formData,title:e.target.value})}/>
                <Select onChange={(e)=>setFromData({...formData,category:e.target.value})}  value={formData.category || ''}>
            ...
            <ReactQuill theme="snow" className='h-96 mt-5 my-16  dark:text-white' placeholder='Write something' value={formData.content || ''} onChange={(value)=>(setFromData({...formData,content:value}))}/>
